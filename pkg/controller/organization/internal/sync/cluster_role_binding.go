...
// NewOwnerClusterRoleBindingSyncer returns a new syncer.Interface for reconciling owner ClusterRoleBinding
func NewOwnerClusterRoleBindingSyncer(org *organization.Organization, cl client.Client, scheme *runtime.Scheme) syncer.Interface {
	obj := org.ComponentObject(organization.OwnerClusterRoleBinding)

	return syncer.NewObjectSyncer("OwnerClusterRole", org.Unwrap(), obj, cl, scheme, func(existing runtime.Object) error {
		out := existing.(*rbacv1.ClusterRoleBinding)
		...
		if out.ObjectMeta.CreationTimestamp.IsZero() {
			...
			out.Subjects = append(out.Subjects, owner)
		}
		out.RoleRef = rbacv1.RoleRef{
			APIGroup: "rbac.authorization.k8s.io",
			Kind:     "ClusterRole",
			Name:     org.ComponentName(organization.OwnerClusterRole),
		}
		return nil
	})
}
